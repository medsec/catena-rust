//! An implementation of Catena-Mydasfly-Full. This variant of Catena provides
//! resistance to ASIC-based adversaries.

/// The choices for H, H', F, Γ and Φ for Catena-Mydasfly-Full.
///
/// These choices are:
///
/// - H: Blake2b
...
pub struct MydasflyFullAlgorithms;

impl ::catena::Algorithms for MydasflyFullAlgorithms {
    ...
    -> Vec<u8> {
        ::components::graph::double_butterfly_graph::double_butterfly_hash(
            self, &garlic, state.to_vec(), lambda, n, k)
    }
...
/// Constructor for a Catena-Mydasfly-Full instance.
pub fn new() -> ::catena::Catena<MydasflyFullAlgorithms> {
    let mff_algorithms = MydasflyFullAlgorithms;
    ::catena::Catena {
        algorithms: mff_algorithms,
        vid: "Mydasfly-Full",
        n: 64,
        k: 64,
        g_low: 18,
...
mod tests {
    ...
    #[test]
    fn mydasflyfull_reduced_test_from_json() {
        let mut test_catena = new();
        test_catena.g_low = 9;
        test_catena.g_high = 9;
        catena_test_from_json(test_catena, "test/test_vectors/catenaMydasflyFullReduced.json");
    }

    #[test]
    #[cfg(feature="fulltest")]
    fn mydasflyfull_test_from_json() {
        let test_catena = new();
        catena_test_from_json(
            test_catena, "test/test_vectors/catenaMydasflyFull.json");
    }
}
